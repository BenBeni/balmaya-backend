...
create table if not exists beneficiary_category_recipients (
  id uuid primary key,
  logical_id uuid not null,
  version int not null,
  is_current boolean not null,
  ...
  recipient_account_number text null,
  provider text not null,

  constraint uq_benef_category_recipient_logical_version unique (logical_id, version)
);
create unique index if not exists uq_benef_category_recipient_current
  on beneficiary_category_recipients(beneficiary_id, category) where is_current = true;
create index if not exists idx_benef_category_recipient_benef
  on beneficiary_category_recipients(beneficiary_id) where is_current = true;
...
create index if not exists idx_schedule_beneficiary on schedules(beneficiary_id) where is_current=true;
